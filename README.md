...
Use the Makefile `all` target to install the dependencies, run the test, autogenerate and build the server an client code. All the generated server code, controller code and `main.go` are found in the `cmd/blued` folder. The generated client code are found in the `cmd/blue` folder.
```sh
$ make
```
Notice that whenever `make` is run, the controllers code are re-generated and output to the `cmd/blued/` folder. These auto-generated controllers code are ignored by git. All the working controllers code are found in the `cmd/blued/ctrl` folder. Any changes to the controllers code (caused by changing the `design/design.go` spec) must be manually copied to the `cmd/blued/ctrl` folder. The `cmd/blued/main.go` can be force-regenerated by deleting it first, and use `git diff` to capture the changes.
